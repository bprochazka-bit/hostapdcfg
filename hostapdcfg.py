...
function renderConfig(text) {
  ...
  el.innerHTML = text.split('\n').map(line => {
    ...
    // Detect annotation: "  # ↓ ..." (legacy — annotations are now emitted
    // on their own line above the directive; this is a safety fallback).
...
def generate_hostapd_conf(params: dict, orig_params: dict = None) -> str:
    ...
    _ann_split = re.compile(r"^(.*?)\s*#\s*↓\s*(.*?)\s*$")
    def c(t=""):
    ...
    if wifi_gen >= 6 and db.get("he_capab"):
        ...
        if db.get("he_bss_color"):
            c(f"he_bss_color={he_bss_color}"
              + (ann("he_bss_color")
                 or derived("BSS coloring allows spatial reuse by distinguishing "
                            "overlapping APs (range 1-63, pick unique value per AP)")))
        c()
